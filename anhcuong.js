...
async extractNextIdData() {
  try {
    const nextIdData = await this.page.evaluate(() => {
      // Lấy cột đầu tiên bên trái (next step trực tiếp)
      const firstColumn = document.querySelector(
        "div.position-relative.careerPaths_column__VCDCr.careerPaths_left__2fK0O"
      );

      const result = [];
      if (firstColumn) {
        const rows = firstColumn.querySelectorAll(
          "div.careerPaths_row__kyPft.careerPaths_left__2fK0O"
        );

        rows.forEach((row) => {
          const infoDiv = row.querySelector(
            "div.careerPaths_info__zncVb.careerPaths_left__2fK0O.text-left"
          );
          if (infoDiv) {
