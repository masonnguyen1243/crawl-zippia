...
export class ZippiaCrawler {
  ...
  async getJobsList() {
    console.log(
      "Starting to crawl Zippia sales industry pages alphabetically..."
    );

    // Define alphabet letters to iterate through
    const alphabetLetters = "abcdefghijklmnopqrstuvwxyz".split("");
    let allJobLinks = [];

    for (const letter of alphabetLetters) {
      const pageUrl = `https://www.zippia.com/sales-industry/${letter}/`;
      console.log(`\nNavigating to page: ${pageUrl}`);
      ...
      try {
        ...
        console.log(`Page ${letter.toUpperCase()} debug info:`, pageInfo);
        ...
        if (jobLinks.length > 0) {
          console.log(
            `Found ${
              jobLinks.length
            } job links on page ${letter.toUpperCase()}:`,
            jobLinks.slice(0, 5).map((j) => j.name) // Show first 5 as sample
          );

          // Filter out duplicates by job name
          const existingNames = new Set(
            allJobLinks.map((job) => job.name.toLowerCase())
          );
          const newJobs = jobLinks.filter(
            (job) => !existingNames.has(job.name.toLowerCase())
          );

          allJobLinks.push(...newJobs);
          console.log(
            `Added ${newJobs.length} new unique jobs. Total so far: ${allJobLinks.length}`
          );
        } else {
          console.log(`No job links found on page ${letter.toUpperCase()}`);
        }
        ...
        console.log(
          `Error crawling page ${letter.toUpperCase()}:`,
          error.message
        );
    ...
    if (allJobLinks.length > 0) {
      console.log(`\n=== SUMMARY ===`);
      console.log(
        `Total unique jobs found across all pages: ${allJobLinks.length}`
      );
      console.log(
        "Sample job names:",
        allJobLinks.slice(0, 10).map((j) => j.name)
      );

      return allJobLinks;
    } else {
      console.log("No job links found across all pages, using fallback...");
      // Fallback to hardcoded jobs if all pages fail
      const jobLinks = [
        {
